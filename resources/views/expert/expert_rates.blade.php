...
<body class="min-h-screen bg-[#121826]">
  ...
  <div class="mx-auto max-w-[420px] min-h-screen bg-[#121826] shadow-2xl shadow-black/50 relative">
    ...
    <!-- Header with back button (navy/gold theme) -->
    <div class="flex items-center justify-between px-4 py-3 bg-[#0B1C3F] border-b border-[#2A3A5A]">
      <a href="{{ route('expert.dashboard') }}" class="p-2 rounded-full bg-[#1A2A4A] text-[#E8EDF2] hover:bg-[#2A5C8C] transition">
    ...
    <!-- ADD RATE FORM (navy/gold theme) -->
    <div class="px-4 mt-2 mb-5">
      <div class="card rounded-xl p-4 bg-[#1A2636] border border-[#2A3A5A] shadow-md">
        <div class="flex items-center gap-2 mb-3">
          <i data-lucide="plus-circle" class="w-5 h-5 text-[#F4A261]"></i>
        ...
        <form action="{{ route('expert.rates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
          ...
          <div>
            ...
            <input type="text" name="service_name" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-[#F4A261]" placeholder="e.g., Electrical Repair, Plumbing, Web Dev">
          </div>
          <div>
          ...
          <div>
            ...
            <input type="file" name="image" accept="image/*" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-[#F4A261] file:mr-2 file:py-1 file:px-3 file:rounded-full file:bg-[#F4A261] file:text-white file:border-0">
            <p class="text-gray-500 text-[10px] mt-1">Max 2MB. JPG, PNG, GIF</p>
          </div>
          <button type="submit" class="w-full py-2.5 rounded-lg bg-gradient-to-r from-[#F4A261] to-[#E08E3E] font-semibold text-white text-sm hover:brightness-110 transition flex items-center justify-center gap-2">
    ...
    <div class="px-4 mb-20">
      ...
      <div class="space-y-3">
        ...
        <div class="bg-[#1A2636] rounded-xl border border-[#2A3A5A] p-3 transition hover:border-[#F4A261]/30">
          <div class="flex gap-3">
            ...
            <div class="flex-1">
              <div class="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span class="bg-[#F4A261]/20 text-[#F4A261] text-[11px] px-2 py-0.5 rounded-full font-mono">Rs. {{ number_format($rate->rate, 0) }}</span>
              </div>
              @if($rate->description)
            ...
            <div class="flex gap-2 items-start">
              <button class="edit-rate-btn p-1.5 rounded-lg bg-gray-800 hover:bg-[#F4A261] transition text-gray-300"
                data-id="{{ $rate->id }}"
      ...
      @else
      <div class="text-center py-10 bg-[#1A2636] rounded-xl border border-dashed border-[#2A3A5A]">
    ...
    <!-- Edit Modal (navy/gold theme) -->
    <dialog id="editRateModal" class="p-0 bg-transparent rounded-xl max-w-md w-full" style="width: 100%; max-width: 330px;">
      <div class="bg-[#1A2636] rounded-xl shadow-2xl border border-[#2A3A5A] p-5 text-white">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-bold flex items-center gap-2"><i data-lucide="edit-3" class="w-5 h-5 text-[#F4A261]"></i> Edit Service Rate</h3>
        ...
        <form id="editRateForm" method="POST" action="" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            <label class="block text-gray-300 text-xs mb-1">Service Name</label>
            <input type="text" name="service_name" id="edit_service_name" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-[#F4A261]">
          </div>
          <div class="mb-3">
            <label class="block text-gray-300 text-xs mb-1">Price (Rs.)</label>
            <input type="number" step="0.01" name="rate" id="edit_rate_value" required class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-2 focus:ring-[#F4A261]">
          </div>
          <div class="mb-4">
          ...
          <div class="flex justify-end gap-3">
            <button type="button" id="cancelEditModal" class="px-4 py-2 bg-gray-700 rounded-lg text-sm hover:bg-gray-600 transition">Cancel</button>
            <button type="submit" class="px-5 py-2 bg-[#F4A261] hover:bg-[#E08E3E] rounded-lg text-sm font-semibold transition">Update Rate</button>
          </div>
        </form>
      </div>
    </dialog>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      window.openEditModal = function(id, name, rateVal, desc) {
        editForm.action = `/expert/update/rates/${id}`;
