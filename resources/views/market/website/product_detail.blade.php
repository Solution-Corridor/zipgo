...
<head>
    ...
    <title>{{ $product[0]->meta_title ?? $product[0]->name }} | Feature Desk</title>
    <meta name="description" content="{{ $product[0]->meta_description ?? '' }}">
    <meta name="keywords" content="{{ $product[0]->meta_keywords ?? '' }}">

    <meta property="og:title" content="{{ $product[0]->meta_title ?? $product[0]->name }}">
    <meta property="og:description" content="{{ $product[0]->meta_description ?? '' }}">

    <!-- Schema.org Structured Data (Product) -->
    @if(!empty($product[0]->page_schema))
    {!! $product[0]->page_schema !!}
    @else
...
<script type="application/ld+json">
{
    ...
    "@type": "Product",
    "name": "{{ addslashes($product[0]->name) }}",
    "image": [
        @php
            $images = [];
    ...
    "description": "{{ addslashes(strip_tags($product[0]->meta_description ?? '')) }}",
    "sku": "{{ $product[0]->product_id ?? '' }}",
    "url": "{{ request()->fullUrl() }}",
    "offers": {
        "@type": "Offer",
        "priceCurrency": "PKR",
    ...
    "ratingValue": "{{ $product[0]->product_ratting }}",  
    "reviewCount": "{{ $product[0]->ratting_count }}"     
}
}
</script>
    @endif
...
<body>
    ...
    <ol class="breadcrumb mb-0 bg-transparent p-0">
        ...
        @if(!empty($product[0]->category_url))
            <li class="breadcrumb-item">
                <a href="{{ url('/' . $product[0]->category_url) }}" class="text-decoration-none text-muted">
                    {{ $product[0]->category_name }}
                </a>
            </li>
        @endif

        @if(!empty($product[0]->sub_category_url))
            <li class="breadcrumb-item">
                <a href="{{ url('/' . $product[0]->category_url . '/' . $product[0]->sub_category_url) }}" class="text-decoration-none text-muted">
        ...
        <li class="breadcrumb-item active text-success" aria-current="page">
            ...
            <div class="mt-3">
                ...
                @if(!empty($product[0]->meta_description))
                <p class="text-muted mt-2 mb-0">{{ $product[0]->meta_description }}</p>
                @endif
            </div>
        </div>
    </section>
                ...
                <div class="gallery-col">
                    ...
                    this.src='/assets/images/favicon.png';"
                        ...
                        <div class="thumbnail-gallery">
                            ...
                            @if($img)
                            <div class="thumbnail-container {{ !$activeSet ? 'active' : '' }}"
                                data-image="{{ url('/uploads/market/products/' . $img) }}">
                                <img class="thumbnail-image"
                                    src="{{ url('/uploads/market/products/' . $img) }}"
                                    alt="{{ $product[0]->name ?? 'Product' }} - {{ $i+1 }}"
                                    title="{{ $product[0]->name ?? 'Product' }} - {{ $i+1 }}"
...
<div class="mb-3">
    ...
    <div id="reviewMessage" class="mt-3"></div>
                ...
                <!-- ==== INFO + CART ==== -->
                <div class="info-col">
                         <div class="h4 mb-0 me-2 product_title fw-bold">{{ $product[0]->name ?? '' }}</div>
                    ...
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <div class="text-muted" style="font-size: 0.95rem;">
                            <a target="_blank" href="/product/{{$product[0]->slug}}/reviews" class="text-primary text-decoration-none fw-medium">
                                <strong>{{ number_format($product[0]->ratting_count ?? 0) }}</strong> reviews
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 text-success small">
                        ...
                        <strong>{{ $positiveFeedback }}% positive feedback</strong> from buyers
                    </div>

                    <!-- Badges -->
                    <div class="product-info mb-3">
                        <span class="badge bg-primary me-2">{{ $product[0]->category_name ?? '' }}</span>
                        <span class="badge bg-primary me-2">{{ $product[0]->sub_category_name ?? '' }}</span>
                        <span class="badge bg-primary">{{ $product[0]->quality_name ?? '' }}</span>
                    </div>

                    <!-- Price -->
                    <div class="price-box mb-3">
                        <div class="price-current">
                            Rs. {{ number_format($product[0]->price ?? 0) }}
                        </div>
                        @if($product[0]->cutting_price ?? 0)
                        <div class="price-old">
                            Rs. {{ number_format($product[0]->cutting_price, 0) }}
                        </div>
                        @endif
                    </div>
                    ...
                    @endphp
                    <div class="stock-info {{ $stockClass }} mb-3">
                        <strong>{{ $stockText }}</strong>
                        <div class="text-muted small mt-1">
                            {{ number_format($sold) }} sold this month
                        </div>
                    </div>
                    ...
                    <div class="qty-selector mb-4">
                        ...
                        <input type="number" class="qty-input" value="1" min="1" max="{{ $left > 0 ? $left : 1 }}">
...
<p style="font-size: 1.0rem; margin-bottom: 0; font-weight: 700; color:#F2AD0C;">
    @auth
    Balance: Rs. {{ number_format(Auth::user()->balance, 0) }}
    @endauth
</p>
<div class="cart-actions">
    <!-- <a href="#" 
       class="btn-buy" 
       id="whatsappOrderBtn" 
       style="display: {{ $left <= 0 ? 'none' : 'block' }}; text-decoration: none;" 
       {{ $left <= 0 ? 'onclick="return false;"' : '' }}>
        WHATSAPP ORDER
    </a> -->
    
    <button class="btn-cart" id="buyNowBtn"
        data-buy-url="{{ route('market.buynow', $product[0]->product_id) }}"
        {{ $left <= 0 ? 'disabled' : '' }}>
        Buy Now
    </button>
</div>
...
<div id="outOfStockMsg" class="text-danger mt-2" style="display: {{ $left <= 0 ? 'block' : 'none' }};">
...
if (whatsappBtn) {
    ...
    const productName = '{{ addslashes($product[0]->name) }}';
    const productPrice = '{{ $product[0]->price ?? 0 }}';
    const productUrl = '{{ request()->fullUrl() }}';
...
submitBtn.addEventListener('click', function() {
    ...
    // Send via AJAX
    fetch('{{ route("market.submit.review") }}', {
        method: 'POST',
        headers: {
