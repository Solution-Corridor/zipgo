...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
              ...
              <div class="card card-primary card-outline card-outline-tabs">
                <div class="card-header p-0 border-bottom-0">
                  ...
                  <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
                    <li class="nav-item">
                      <a class="nav-link active" id="custom-tabs-four-home-tab" data-toggle="pill" href="#custom-tabs-four-home" role="tab" aria-controls="custom-tabs-four-home" aria-selected="true"><i class="fa fa-list"></i>&nbsp;&nbsp;List Category</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link" id="custom-tabs-four-profile-tab" data-toggle="pill" href="#custom-tabs-four-profile" role="tab" aria-controls="custom-tabs-four-profile" aria-selected="false"><i class="fa fa-plus"></i>&nbsp;&nbsp; Add Category</a>
                    </li>
                  </ul>
                </div>
                <div class="card-body">
  ...
  <tbody>
    ...
    <tr>
      <td>{{ $category->id }}</td>
      <td>
    <a href="{{ asset($category->feature_image) }}" target="_blank">
      ...
      <td>{{ $category->product_count }}</td> <!-- Show count -->
      <td>{{ \Carbon\Carbon::parse($category->created_at)->format('d-m-Y') }}</td>
      <td>
    {{-- Meta Title --}}
    @php
        $titleCount = strlen($category->meta_title ?? '');
        $descriptionCount = strlen($category->meta_description ?? '');
    @endphp
...
<br>
    ...
    @endif
      ...
      <td>
        <a href="{{ route('market.edit_category', $category->id) }}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
        <form action="{{ route('market.delete_category', $category->id) }}" method="POST" style="display:inline-block;">
          @csrf
          @method('DELETE')
          <button type="submit" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure you want to delete this category?');"><i class="fa fa-trash"></i></button>
        </form>
      </td>
    </tr>
    @endforeach
                    ...
                    <div class="tab-pane fade" id="custom-tabs-four-profile" role="tabpanel" aria-labelledby="custom-tabs-four-profile-tab">
                      ...
                      <div class="card card-primary">
                        <!-- form start -->
                        <form action="{{ route('market.save_category') }}" method="POST" enctype="multipart/form-data">
    <div class="card-body">
        <div class="row">
            @csrf
            <!-- Category Name -->
            <div class="col-md-4">
                <label><b>Category Name </b><span class="strdngr">*</span></label>
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="url" class="form-control" placeholder="army-belt" value="{{ old('url') }}" required>
                @error('url')
                <p class="help-block text-danger">{{ $message }}</p>
                @enderror
            </div>
...
<script>
    ...
    <div class="card-footer text-center">
        ...
        <button type="submit" name="btnClass" class="btn btn-primary">Add Category</button>
    </div>
</form>
