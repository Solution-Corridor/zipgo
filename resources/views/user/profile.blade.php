...
<body class="min-h-screen bg-[#0A0A0F]">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Username availability check (unchanged)
            const usernameInput = document.getElementById('username');
            const feedback = document.getElementById('username-feedback');
            const errorDiv = document.getElementById('username-error');
            if (usernameInput) {
                usernameInput.addEventListener('input', function() { this.value = this.value.replace(/\s+/g, ''); clearFeedback(); });
                usernameInput.addEventListener('blur', async function() {
                    const username = this.value.trim();
                    const original = "{{ auth()->user()->username ?? '' }}";
                    if (!username || username === original) { clearFeedback(); return; }
                ...
                function clearFeedback() { feedback.innerHTML = ''; errorDiv.textContent = ''; usernameInput.classList.remove('border-red-500', 'border-green-500/50'); }
                function showError(msg) { feedback.innerHTML = '<i class="fas fa-times-circle text-red-400"></i>'; errorDiv.textContent = msg; usernameInput.classList.add('border-red-500'); }
            }
