          ...
          <div class="p-6">
            ...
            <div class="flex items-start gap-5 mb-8">
              <div class="relative group flex-shrink-0">
                ...
                @if (auth()->user()->pic)
                <img id="avatar-preview" src="/{{ auth()->user()->pic }}" class="w-28 h-28 rounded-2xl object-cover shadow-sm ring-2 ring-[#FF6B6B]/40" alt="Profile photo">
                <div id="avatar-initials" class="hidden w-28 h-28 rounded-2xl bg-gradient-to-br from-[#FF6B6B] to-[#4ECDC4] flex items-center justify-center text-4xl font-bold shadow-sm text-white">
                  {{ strtoupper(substr(auth()->user()->username ?? 'U', 0, 1)) }}
                </div>
                @else
                <div id="avatar-initials" class="w-28 h-28 rounded-2xl bg-gradient-to-br from-[#FF6B6B] to-[#4ECDC4] flex items-center justify-center text-4xl font-bold shadow-sm text-white">
                  {{ strtoupper(substr(auth()->user()->username ?? 'U', 0, 1)) }}
                </div>
                <img id="avatar-preview" src="" class="hidden w-28 h-28 rounded-2xl object-cover shadow-sm" alt="Profile preview">
                @endif
                <label for="avatar" class="absolute inset-0 cursor-pointer rounded-2xl opacity-0 group-hover:opacity-100 bg-black/50 flex items-center justify-center transition-all duration-300">
              ...
              <div class="flex-1 min-w-0 pt-1">
                ...
                <p class="text-xs text-[#6B5B50] mt-4">Since {{ auth()->user()->created_at->format('d M Y') }}</p>
                <p class="text-[10px] text-[#4ECDC4] mt-1 flex items-center gap-1"><i data-lucide="info" class="w-3 h-3"></i> Tap to change photo</p>
              </div>
            </div>
