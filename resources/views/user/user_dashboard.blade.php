        ...
        <div class="flex items-center justify-between">
            ...
            <div>
                ...
                <div id="balanceAmount" 
                    ...
                    $ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $user->balance / $dollarPrice : 0, 2) }}
                </div>
                <div id="balanceAmount" 
...
<div class="px-4 mt-1 mb-4">
    <div class="grid grid-cols-2 gap-3">
        ...
        <!-- Share Balance -->
        @if($user->is_balance_sharing_allowed)
...
@if($isCooldownActive)
<script>
    const nextActionTime = new Date("{{ $nextWithdrawTime->toIso8601String() }}").getTime();
   ...
   class="relative flex-1 min-w-0 py-3 px-4 rounded-xl text-center font-bold text-sm leading-tight
    ...
    <span class="relative z-10 text-black font-bold tracking-wide">
        ...
        <div class="grid grid-cols-3 gap-3 px-4 mb-4">
            ...
            <!-- Tasks / Missions -->
            <a href="{{ route('my_tasks') }}" class="card rounded-xl p-3 text-center hover:scale-105 active:scale-95 transition block focus:outline-none focus:ring-2 focus:ring-green-500">
                ...
                <span class="absolute -top-1 -right-1 bg-amber-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow">{{$myTasksToday}}</span>

                <!-- <span class="absolute -bottom-1 -right-1 bg-green-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow">Total {{$myTasksTotal}}</span> -->
            </a>
            ...
            @if($user->is_complaint_allowed==1)
            <a href="{{ route('my_complaints') }}" class="card rounded-xl p-3 text-center hover:scale-105 active:scale-95 transition block focus:outline-none focus:ring-2 focus:ring-green-500">
        ...
        <div class="mx-4 mb-4 card rounded-xl p-4">
            ...
            <div class="bg-gray-900/60 rounded-lg p-3 mb-1 text-center text-xs break-all" id="referral-link">
                {{ env('APP_URL') }}/register/26{{ auth()->user()->id ?? 'N/A' }}
            </div>
        ...
        <div class="px-4 grid grid-cols-2 gap-3 mb-2">
            ...
            <div class="card rounded-xl p-3.5 text-center">
                ...
                <p class="text-xl font-bold text-emerald-400">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $total_deposit / $dollarPrice : 0, 2) }}
                </p>

                <p class="text-xs text-gray-400 mb-0.5 mt-1">Tasks Reward</p>
                <p class="text-xl font-bold text-yellow-400">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $tasksReward / $dollarPrice : 0, 2) }}</p>
            </div>
            <div class="card rounded-xl p-3.5 text-center">
                <p class="text-xs text-gray-400 mb-0.5">Total Withdraw</p>
                <p class="text-xl font-bold text-red-400">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $total_withdraw / $dollarPrice : 0, 2) }}</p>

                <p class="text-xs text-gray-400 mb-0.5 mt-1">Balance Shared</p>
                <p class="text-xl font-bold text-yellow-400">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $balanceTransfer / $dollarPrice : 0, 2) }} 
                </p>
            </div>

            <div class="card rounded-xl p-3.5 text-center">
                <p class="text-xs text-gray-400 mb-0.5">Refer Bonus</p>
                <p class="text-xl font-bold text-purple-300">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $refer_bonus / $dollarPrice : 0, 2) }}</p>
            </div>
            <div class="card rounded-xl p-3.5 text-center">
                <p class="text-xs text-gray-400 mb-0.5">Pending Withdraw</p>
                <p class="text-xl font-bold text-cyan-300">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $total_withdraw_pending / $dollarPrice : 0, 2) }}</p>
            </div>

            <div class="card rounded-xl p-3.5 text-center">
                <p class="text-xs text-gray-400 mb-0.5">Team Size</p>
                <p class="text-xl font-bold">{{ $team_size }}</p>
            </div>
            <div class="card rounded-xl p-3.5 text-center">
                <p class="text-xs text-gray-400 mb-0.5">Team Invest</p>
                <p class="text-xl font-bold">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? $team_invest / $dollarPrice : 0, 2) }}</p>
            </div>
        ...
        <!-- Network Stats - compact -->
            ...
            <div class="bg-gradient-to-r from-purple-900/60 to-blue-900/60 rounded-lg p-3 text-center mx-4 mb-20 card rounded-xl p-4">
                <p class="font-semibold text-base">$ {{ number_format(($dollarPrice && $dollarPrice != 0) ? ($refer_bonus + $dailyProfit + $tasksReward) / $dollarPrice : 0, 2) }}</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const balance = parseFloat('{{ auth()->user()->balance ?? 0 }}') || 0;
