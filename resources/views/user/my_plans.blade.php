...
<body class="min-h-screen bg-[#0A0A0F]">
  ...
  <div class="mx-auto max-w-[420px] min-h-screen bg-[#0A0A0F] shadow-2xl shadow-black/50 relative">
    ...
    <div class="px-4 pt-4 pb-24">
          ...
          <div class="card rounded-xl overflow-hidden glow bg-gradient-to-b from-gray-900/80 to-gray-950/80 border border-purple-800/30">
            <div class="p-4">
              <div class="flex justify-between items-start mb-3">
                <div>
                  <h3 class="font-semibold text-lg">{{ $plan->package_name }}</h3>
                  <p class="text-xs text-purple-400 mt-0.5">
                    Invested: Rs. {{ number_format($plan->amount, 2) }}
                  </p>
                </div>


                <span class="text-xs px-2.5 py-1 bg-gray-600/60 text-gray-300 rounded-full">{{ $plan->duration_days }} days</span>
              ...
              <div class="grid grid-cols-2 gap-3 text-xs mb-4">
                ...
                <div>
                  ...
                  <p class="font-medium text-green-400">{{ $plan->daily_tasks }} x {{ $plan->daily_task_price }} Rs</p>
                </div>
                @if($plan->free_spins > 0)
                <div>
                  <p class="text-gray-400">Free Spins</p>
                  <p class="font-medium">{{ $plan->free_spins }} x {{ $plan->free_spin_price }} Rs</p>
                </div>
                @endif

                <div>
                  <p class="text-gray-400">Daily Profit</p>
                  <p class="font-medium text-green-400">{{ $plan->daily_profit_display }} Rs</p>
                </div>

                @if($plan->weekend_reward > 0)
                <div>
                  <p class="text-gray-400">Weekend Reward</p>
                  <p class="font-medium text-green-400">{{ $plan->weekend_reward }} - {{ number_format($plan->weekend_reward * 0.15 + $plan->weekend_reward) }} Rs</p>
                </div>
                @endif

                <div>
                  <p class="text-gray-400">Started on</p>
                  <p class="font-medium">{{ \Carbon\Carbon::parse($plan->approved_at)->format('d M Y • h:i A') }}</p>
                </div>

                <div>
                  <p class="text-gray-400">Expires</p>
                  <p class="font-medium text-amber-400">
                    {{ $plan->expires_at->format('d M Y • h:i A') }}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p class="font-medium text-amber-400">
                    {{ $plan->days_remaining }} day{{ $plan->days_remaining == 1 ? '' : 's' }}
                  </p>
                </div>

                <div class="mt-2">
                  <form action="{{ route('plan.cancel', $plan->payment_id) }}" method="POST"
              ...
              <div class="h-2 bg-gray-700 rounded-full overflow-hidden">
                <div class="h-full bg-green-400" style="width: {{ $progressPercentage }}%"></div>
              </div>
            </div>
          </div>
          @endforeach
            ...
            <div class="p-4">
              ...
              <div class="text-sm space-y-1 mb-4">
                <div class="flex justify-between">
                  ...
                  <span>Rs. {{ number_format($plan->amount, 0) }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-400">Submitted:</span>
                  <span>{{ \Carbon\Carbon::parse($plan->created_at)->diffForHumans() }}</span>
                </div>
              </div>
            ...
            <div class="p-4">
              ...
              <div class="text-sm space-y-1 mb-3">
                ...
                <div class="flex justify-between">
                  ...
                  <span>{{ \Carbon\Carbon::parse($plan->approved_at)->format('d M Y • h:i A') }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-400">Expired on:</span>
                  <span>{{ \Carbon\Carbon::parse($plan->expires_at)->format('d M Y • h:i A') }}</span>
                </div>
              </div>
        ...
        <div class="space-y-4">
          ...
          <div class="card rounded-xl overflow-hidden bg-gray-900/60 border border-red-800/40 opacity-85">
            <div class="p-4">
              ...
              <p class="text-sm mb-2">Amount: <strong>Rs. {{ number_format($plan->amount, 0) }}</strong></p>
              @if($plan->admin_note)
              <p class="text-xs text-red-300 mb-3">Note: {{ $plan->admin_note }}</p>
              @endif
              <p class="text-xs text-gray-500">
                {{ \Carbon\Carbon::parse($plan->updated_at)->diffForHumans() }}
              </p>
            </div>
          </div>
          @endforeach
      ...
      @if($active->isEmpty() && $pending->isEmpty() && $rejected->isEmpty() && $expired->isEmpty())
