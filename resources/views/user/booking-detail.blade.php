...
<body class="min-h-screen bg-[#e6e5e3]">
  ...
  <div class="mx-auto max-w-[420px] min-h-screen bg-[#e6e5e3] relative">
    ...
    <div class="mx-4 mt-4 bg-white border border-[#EAE0D5] rounded-xl p-4 space-y-3 shadow-sm">
      <div>
        <p class="text-xs text-[#6B5B50]">Service</p>
        <p class="text-[#2C1810] font-medium">{{ $booking->service }}</p>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Professional</p>
        <p class="text-[#2C1810] font-medium">{{ $booking->professional }}</p>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Date & Time</p>
        <p class="text-[#2C1810] font-medium">{{ $booking->date }}</p>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Address</p>
        <p class="text-[#2C1810] font-medium">{{ $booking->address }}</p>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Price</p>
        <p class="text-[#FF6B6B] font-bold text-lg">Rs. {{ $booking->price }}</p>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Status</p><span class="inline-block px-2 py-1 rounded-full text-xs bg-green-100 text-green-700">{{ $booking->status }}</span>
      </div>
      <div>
        <p class="text-xs text-[#6B5B50]">Description</p>
        <p class="text-sm text-[#2C1810]">{{ $booking->description }}</p>
      </div>
    </div>
